...
export type ConferenceContextWithSetter =
  ConferenceContext & { addTalk: (talk: Talk) => void };
...
export const ConferenceContext =
  React.createContext<ConferenceContextWithSetter>(
  ...
  return (
    <ConferenceContext.Provider
      value={{
        ...talksContextState,
        addTalk(talk: Talk) {
          const openTracks = this.openTracks;

          if (openTracks.length === 0) {
            _createNewTrackAndAdd(
              talk,
              talksContextState.tracks,
            );
          ...
          const successful =
            firstOpenTrack.key.addTalk(talk);

          if (!successful) {
            firstOpenTrack.value = true;
            return;
          }
